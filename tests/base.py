...
class BaseTestClass:
    ...
    def setup_class(cls):
        ...
        cls.pdf_files = [
            (cls.cams_file_name, cls.cams_password, 21, 29),
            (cls.kfintech_file_name, cls.kfintech_password, 20, 26),
        ]
    ...
    def test_output_json(self):
        for filename, password, num_folios, _ in self.pdf_files:
            ...
            assert data.get("investor_info", {}).get(
                "mobile") not in (None, "")
    ...
    def test_read_dict(self):
        ...
        for pdf_file, pdf_password, _, num_schemes in self.pdf_files:
            ...
            assert re.search(
                rf"Matched\s+:\s+{num_schemes}\s+schemes", clean_output) is not None
            assert re.search(r"Error\s+:\s+0\s+schemes",
                             clean_output) is not None
